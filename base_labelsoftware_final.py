# labelsoftware_final.py
# Versión 3.0 - Integral: Excel, CSV, licencias, interfaz mejorada
...
class EtiquetadoraApp:
    ...
    def cambiar_idioma(self):
            ...
            if isinstance(child, tk.Frame):
                for subchild in child.winfo_children():
                    if isinstance(subchild, ttk.Label):
                        ...
                        if texto_actual in mapeo_etiquetas:
                            subchild.config(text=t[mapeo_etiquetas[texto_actual]])
        ...
        # Botones sin referencia directa
    ...
    def pegar_desde_excel(self):
        try:
            ...
            for idx_fila, linea in enumerate(lineas[inicio:], start=1):
                ...
                if len(celdas) < 7:
                    errores.append(f"Fila {idx_fila}: {len(celdas)} columnas, se esperaban al menos 7")
                    continue
                proveedor = celdas[0].strip()
                idProd = celdas[1].strip() if len(celdas) > 1 else ""
                ...
                # Limpiar gramos
                try:
                    g = int(float(gramos_raw.replace(',', ''))) if gramos_raw else 0
                    gramos = f"{g:03d}"
                except:
                    gramos = "000"
            ...
            if nuevos:
                ...
                self.actualizar_tabla()
                total_etiquetas = sum(p['cantidad'] for p in nuevos)
                total_valor = sum(float(p['precio']) * p['cantidad'] for p in nuevos)
                msg = f"✅ Se agregaron {len(nuevos)} productos.\n"
                msg += f"🏷️ Total de etiquetas: {total_etiquetas}\n"
                msg += f"💰 Valor total: ${total_valor:,.0f}"
                if errores:
                    msg += f"\n⚠️ {len(errores)} errores (revisa el log)"
